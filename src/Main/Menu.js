...
import { Cancel, Flag, Group, GroupSharp, Home, Notifications, Settings, Store, VideoLibrary } from '@material-ui/icons';
...
function Menu() {
  ...
  return (
    ...
    <MenuWrapper>
      ...
      <MenuContent>
        <ProfileBar>
          <Avatar src='images/Atunu.jpg' style={{backgroundClip:'content-box'}}/>
          <UserInfo>
            <h2>Makinde Mayowa</h2>
            <Link to={'./profile'}><span>View your Profile</span></Link>
          </UserInfo>
        </ProfileBar>
          <ul>
          {
            MenuBars.map(bars=>(
              <li key={bars.id}>
                {
                  (bars.funct)?
                  (<div onClick={bars.funct}>{bars.icon}<span>{bars.tabname}</span></div>):
                  (<Link to={bars.linkPath}><div>{bars.icon}<span>{bars.tabname}</span></div></Link>) }
              </li>
              ))
            }
          </ul>
